...
## The context measurement is now the guard's own former formula, adopted

The claim: `bin/fm-context-measure-lib.sh` does not keep a second, weaker measurement of its own.

`fm-context-measure-lib-adoption` (docs/verification/context-budget.md) completed the move this record originally anticipated: `fm_context_measure_transcript` is now `bin/fm-context-budget.sh`'s former streaming `measure_context`, lifted verbatim, and that guard now sources this lib instead of keeping its own copy.
The lib prints `"TOTAL COMPACTS"`; `bin/fm-session-pulse.sh` reads the field before the first space as `TOTAL` and does not need the compaction tally.

Recorded 2026-08-28 on macOS 26.5.1 (arm64), bash 5.3.0, jq 1.8.2, git 2.55.0, over a real transcript:

```console
$ T=~/.claude/projects/-Users-uayyagari-workspace-applypass/fff741b2-4027-4ce5-8ff0-feaa46390ea9.jsonl
...
The largest transcript on this machine today is a genuine working session:

```console
$ BIG=$(ls -S ~/.claude/projects/*/*.jsonl | head -1)
$ ls -lh "$BIG" | awk '{print $5, $9}'
4.8M $HOME/.claude/projects/<encoded-project-dir>/<session-id>.jsonl
$ time bash -c '. bin/fm-context-measure-lib.sh; fm_context_measure_transcript "$1"' _ "$BIG"
258458 0
bash -c ...  0.03s user 0.00s system 97% cpu 0.039 total
```

258,458 tokens is above the threshold, and the measurement costs 39 ms at this size on this host; the 2026-08-05 record found a 45 MB session costing 1.6 s, which remains the operative worst-case cost estimate since no transcript this large exists on this host today.
