...
## The lib now owns the measurement (`fm-context-measure-lib-adoption`, resolved)

A review of this branch once asked it to source `bin/fm-context-measure-lib.sh` and delete the inline `measure_context`, and was declined twice because a straight swap at the time would have deleted working behavior: the lib slurped with `jq -s` instead of streaming, had no compaction tally, and deduped a multi-block turn by `requestId` instead of taking the last entry outright.

That gap is closed.
`bin/fm-context-measure-lib.sh`'s `fm_context_measure_transcript` is now this guard's own former `measure_context`, lifted verbatim: one streaming `jq -R` pass reduced by `awk`, printing `"TOTAL COMPACTS"` so a caller gets the compaction tally alongside the total from the same pass.
This guard sources the lib and no longer keeps a private copy; `bin/fm-session-pulse.sh` already sourced the lib and now reads `TOTAL` as the field before the first space in that same output, discarding the compaction tally it does not need.

Recorded 2026-08-28 on macOS 26.5.1 (arm64), bash 5.3.0, jq 1.8.2, git 2.55.0.
...
The requestId-dedup question this table used to track is resolved as "no functional difference": `(.[-1].requestId) as $rid | [.[] | select($rid == null or .requestId == $rid)][-1]` always reduces to `.[-1]` outright, because `$rid` is defined as the last element's own requestId, so the array filtered down to that requestId trivially still ends with that same last element.
`tests/fm-context-measure-lib.test.sh` pins the now-genuine version of that contract with two different requestIds, not one shared one, so a rewrite that reintroduced real grouping semantics would be caught.

`tests/fm-context-budget.test.sh`'s structural streaming assertion now targets the lib file, not the guard, and separately asserts the guard contains no `jq -R` pass of its own - the one-owner rule enforced as a test, not just a comment.
