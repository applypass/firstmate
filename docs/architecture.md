...
Ship briefs also tell the crewmate to verify `pwd -P` and `git rev-parse --show-toplevel` before creating its branch, then stop with a blocked status if it landed in the primary checkout.
...
`data/projects.md` records each project's delivery mode plus its optional `+yolo` autonomy flag and `+ticket:<prefix>` ticket mandate, all parsed by `bin/fm-project-mode.sh`.
