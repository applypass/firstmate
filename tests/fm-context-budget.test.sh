...
install_budget_scripts() {
  ...
  cp "$ROOT/bin/fm-context-measure-lib.sh" "$dir/bin/fm-context-measure-lib.sh"
...
#
# bin/fm-context-measure-lib.sh owns the reader now (fm-context-measure-lib-
# adoption); this asserts BOTH that the shared lib still streams, and that the
# guard is a genuine one-owner caller with no reintroduced copy of its own.
test_measurement_never_slurps_the_transcript() {
  local lib="$ROOT/bin/fm-context-measure-lib.sh"
  local guard="$ROOT/bin/fm-context-budget.sh"
  local passes
  assert_no_grep 'jq -s' "$lib" \
    "the reader must stream the transcript, never slurp it into one array"
  assert_no_grep 'jq --slurp' "$lib" \
    "the reader must stream the transcript, never slurp it into one array"
  passes=$(grep -c '^[^#]*jq -R' "$lib" || true)
  [ "$passes" -eq 1 ] \
    || fail "the transcript must be parsed by exactly one pass, not $passes"
  pass "fm-context-measure-lib: the measurement is one streaming pass at constant memory"

  assert_no_grep 'jq -R' "$guard" \
    "the guard must not keep its own copy of the transcript reader"
  assert_contains "$(cat "$guard")" 'fm_context_measure_transcript' \
    "the guard must call the shared measurement, not reimplement it"
  pass "fm-context-budget: the guard has no private copy of the measurement"
}
