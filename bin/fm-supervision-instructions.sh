...
repair_line() {
  ...
  case "$HARNESS" in
    ...
    grok)
      ...
      ;;
    cursor|cursor-agent)
      printf '%s%s\n' "$prefix" 'watcher supervision is owned by the Cursor Shell command stop hook; inspect its bin/fm-watch-arm.sh registration and startup path before ending the turn.'
      ;;
    *)
...
ordinary_wake_line() {
  case "$HARNESS" in
    ...
    grok)
      ...
      ;;
    cursor|cursor-agent)
      printf '%s\n' '- Ordinary wake: the Cursor Shell command stop hook re-arms bin/fm-watch-arm.sh automatically; drain and handle the wake, and do not arm another cycle yourself.'
      ;;
    *)
