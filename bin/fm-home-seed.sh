...
clone_project() {
  ...
  if ! mode=$(FM_HOME="$FM_HOME" FM_DATA_OVERRIDE="$DATA" "$FM_ROOT/bin/fm-project-mode.sh" "$project"); then
    echo "error: cannot resolve delivery mode for $project; fix the registry bracket" >&2
    return 1
  fi
  mode=${mode%% *}
...
project_mode_in_home() {
  ...
  if ! mode=$(FM_ROOT_OVERRIDE='' FM_STATE_OVERRIDE='' FM_DATA_OVERRIDE='' FM_PROJECTS_OVERRIDE='' FM_CONFIG_OVERRIDE='' FM_HOME="$home" "$FM_ROOT/bin/fm-project-mode.sh" "$project"); then
    return 1
  fi
  mode=${mode%% *}
  printf '%s\n' "$mode"
