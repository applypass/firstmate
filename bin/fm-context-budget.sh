...
#
# WHAT IT MEASURES, AND THE FORMULA: bin/fm-context-measure-lib.sh is the single
# owner of the measurement (transcript_path handling, the token-total formula,
# the three correctness rules, and the streaming/constant-memory guarantee).
# This guard sources it rather than keeping its own copy; only the policy below
# - the ceiling, the two stages, warning vs. enforcement, and the record
# lifecycle - is this file's own.
# The measurement's TIME is still linear in the transcript's size - roughly
# 0.7 s per 80 MB on the measured host - and that cost is paid at every single
# turn end, including far below the advisory.
#
...
# --- the measurement ---------------------------------------------------------
# bin/fm-context-measure-lib.sh owns the formula, the streaming/constant-memory
# pass, and the compaction tally this guard's genuine-reset detection needs
# (record_predates_a_compaction below); see fm_context_measure_transcript.
MEASURED=$(fm_context_measure_transcript "$TRANSCRIPT") || exit 0
TOTAL=${MEASURED%% *}
COMPACTS=${MEASURED##* }
case "$TOTAL" in
  ''|*[!0-9]*) exit 0 ;;
