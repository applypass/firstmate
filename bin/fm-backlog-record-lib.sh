...
# ONE owner of the canonical backlog record model and of "which recorded pull
# request belongs to a task".
#
# WHY IT IS A LIB: bin/fm-fleet-snapshot.sh owns these two contracts, and every
# other surface must consume them rather than restate them. A second inline
# definition of an actionable captain hold drifts from this one silently, and a
# reader has no way to tell which of the two is the rule. bin/fm-awaiting-captain.sh
# needs both at every session start and cannot afford the full snapshot, so the
# predicates live here and both callers source them.
#
# fm_backlog_records_json is the only place that decides captain_actionable: a
# hold waits on the captain only when it is QUEUED, kind captain, hold-kind
# captain, carries a hold reason, and has no unresolved blocker. An IN-FLIGHT
...
# fm_first_pr_url_in_file <file>: the first pull request URL recorded anywhere in
# a file, used to read a PR out of a task's status stream.
fm_first_pr_url_in_file() {  # <file>
  [ -f "$1" ] || return 1
