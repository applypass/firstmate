...
- **Explicit project modes** - each project ships via `no-mistakes`, `direct-PR`, or `local-only`, with an optional `+yolo` autonomy flag and an optional `+ticket:<prefix>` flag that mandates a tracker ticket per change and keys the crew branch and PR-title convention.
